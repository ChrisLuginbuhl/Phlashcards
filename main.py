...
# TODO 22: Check for crashes when no connection to db or img_urls

card_datafield_names = [
    "rec_id",
    "card_id",
    "num_views",
    "initial_frequency",
...
class Schedule:
    ...
    def fill_queue(self):
        ...
        try:
            ...
            logger.debug(
                f"Retrieved Card_data from db: {len(card_data)} items. First item: {card_data[0]}"
            )
        ...
        initial_frequencies = [
            card["initial_frequency"]
            if "initial_frequency" in card.keys()
            else gc.INITIAL_FREQUENCY_DEFAULT
        ...
        skip_until = [
            card["skip_until"]
            if "skip_until" in card.keys()
            else gc.SKIP_UNTIL_DATE_DEFAULT
            for card in card_data
        ]
        archived = [
            card["archived"] if "archived" in card.keys() else False
            for card in card_data
        ]
        weights = [
            get_weight(init_freq, num_views[idx], frequency_decay[idx])
            for idx, init_freq in enumerate(initial_frequencies)
        ]
        ...
        self.eligible_cards = [
            ...
            if card.card_id not in ids_of_cards_in_queue
            and not card.archived
            and (dt.datetime.strptime(card.skip_until, "%Y-%m-%d").date() - today)
            < dt.timedelta(days=1)
            and not (
                not card.tags
                or set(card.tags.split(" ")).intersection(set(self.excluded_tags))
            )
        ...
        logger.info(
            f"Eligible cards are: {sorted([card.card_id for card in self.eligible_cards])}"
        )
        # Now fill the queue
        # self.queue = []
        self.queue.clear()
        for i in range(gc.QUEUE_SIZE):
            try:
                self.queue.append(
                    random.choices(
                        self.eligible_cards,
                        [card.weight for card in self.eligible_cards],
                        k=1,
                    )[0]
                )
                ...
                logger.error(
                    f"Index Error occurred while filling the queue. Possibly not enough cards to fill "
    ...
    def skip_card(self, card_id: int, days_to_skip: int):
        ...
        try:
            queue_position = [
                i for i, card in enumerate(self.queue) if card.card_id == card_id
            ][0]
            logger.debug(f"Found card to skip in queue position {queue_position}")
            self.queue[queue_position] = self.queue[queue_position]._replace(
                skip_until=str(dt.date.today() + dt.timedelta(days=days_to_skip))
            )
            logger.debug(
    ...
    def update_db(self):
        updates_list = [
            {
                "id": card.rec_id,
                "fields": {
                    "num_views": card.num_views + 1,
                    "skip_until": card.skip_until,
                },
            }
        ...
        try:
            ...
            logger.error("Connection Error: Unable to reach database. Number of views not updated")
            flash("Error connecting to database. Number of views not updated")
    ...
    if image_url:
        try:
            ...
            if r.headers["content-type"] in image_formats:
                ...
                logger.error(
                    f'URL does not link to an image of type {"".join(image_formats)}'
                )
            ...
            logger.error(
                f'URL is not complete (may need "https://"). URL is {image_url}'
            )
    else:
...
@logger.catch()
def default_if_none(
    freq_decay=gc.FREQUENCY_DECAY_DEFAULT,
    n_views=0,
...
def get_weight(initial_freq: int, num_views: int, decay_rate: int) -> float:
    ...
    try:
        weight = initial_freq * math.e ** -(
            num_views / (gc.FREQUENCY_DECAY_RATE_MAX - decay_rate + 1)
        )
...
def add_missing(card: dict) -> dict:
    ...
    updated_card = {
        k: card["fields"][k] if k in card["fields"].keys() else None
        for k in card_datafield_names
    }
    updated_card["rec_id"] = card[
        "id"
    ]  # returning a flattened dict where 'rec_id' is placed at same level as 'card_id'
    return updated_card
...
def load_user(id):
    try:
        ...
        logger.error(f"Connection Error. Unable to load user from id {id}")
    return user
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if user_data_raw:
            ...
            logger.error(
                f'Trying to add email: {request.form.get("email")}. Found: {user_data["email"]}'
            )
...
def login():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            logger.error(
                "Connection error. Error connecting to database (e.g too many retries"
            )
            flash("Error connecting to database")
            return redirect(url_for("login"))
        user = User(
...
def show_card():
    ...
    if skip_form.validate_on_submit():
        logger.debug(
            f"received back from form - card: {skip_form.card_id.data}, type: {type(skip_form.card_id.data)}, "
    ...
    logger.debug(
        f'Card data passed to template: Card {requested_card["card_id"]}: {requested_card["title"]}'
    )
    return render_template(
        "card_and_image.html",
        card=requested_card,
...
def get_all_cards():
    try:
        ...
        if card_data_raw:
            ...
            all_tags_nested = [
                card["tags"].split(" ") for card in card_data if card["tags"]
            ]  # a list of lists
            all_tags = set(
                [item for sublist in all_tags_nested for item in sublist]
            )  # Produces set from flattened list
...
def add_new_card():
    ...
    if form.validate_on_submit():
        ...
        title, body, img_url = sanitize(
            form.title.data, form.body.data, form.img_url.data
        )
        body = body.replace("\n\n\n", "\n\n")
        logger.debug(
            f"Form returned: frequency_decay {form.frequency_decay.data}, num_views {form.num_views.data}, "
        ...
        (
            ...
            form.frequency_decay.data,
            form.num_views.data,
            form.initial_frequency.data,
            form.skip_until.data,
        )
        skip_until = skip_until_response.strftime("%Y-%m-%d")
        tags = clean(form.tags.data, strip=True)
        logger.debug(
            f"Attempting to create card with title: {title}, img_url: {img_url}, author "
            f"{current_user.user_name}, freq_decay: {frequency_decay}, tags: {tags}, "
        ...
        try:
            ...
            logger.error(
                f"Connection Error. Unable to connect to database. Response {response}"
            )
...
def edit_card(card_id):
    # Get existing card data from db and make the edit form
    ...
    # Update the db with the edited card data
    if edit_form.validate_on_submit():
        title, body, img_url = sanitize(
            edit_form.title.data, edit_form.body.data, edit_form.img_url.data
        )
        (
            frequency_decay,
            num_views,
        ...
        try:
            ...
            logger.error(
                f"Connection Error. Unable to connect to database."
            )
